...
struct REEDNode {
    ...
    struct REEDNode *children[ 4 ]; /* child nodes */
...
/* create a r-eed tree return 0 on success, 1 else */
{
  ...
  for( i = 0; i < level; i++ ) {
    ...
    if( i == 0 ) {
      ...
      tree->nodes[ i ][ 0 ].children[ 2 ] = NULL;
      tree->nodes[ i ][ 0 ].children[ 3 ] = NULL;
      ...
      tree->nodes[ i ][ 0 ].children[ 3 ] = NULL;
        ...
        tree->nodes[ i - 1 ][ j / 4 ].children[ j % 4 ] =
          &tree->nodes[ i ][ j ];
      }
...
FILE* debug_file, BFILE* fp, long *num_symbols, long *source_length, long *ret_cache) {
  ...
  REEDNode node,node1,node2,node3,node4;     /* current node */
...
/*  while(avg_thres>threshold)
  ...
  cx=tree->nodes[0][0].cx;
  cy=tree->nodes[0][0].cy;
  nx=tree->nodes[0][0].nx;
  ny=tree->nodes[0][0].ny;
  fprintf(debug_file, "\n (%ld,%ld) (%ld,%ld) (%ld,%ld) (%ld,%ld)",cx,cy,cx+nx-1,cy,cx,cy+ny-1,cx+nx-1,cy+ny-1);
  for( i = 1; i < (long)(tree->level); i++ ) {
    n = pow( 4.0, i );
      ...
      //  fprintf(debug_file, "2 ");
        parent = (tree->nodes[i-1][(int)floor(j/4)]);
      //  printf("%d \n", parent.value);
      ...
      //  
          ...
          if(j%4==0)
            ...
            {cx=parent.cx+nx_new; cy=parent.cy; nx_new++;}

          else if(j%4==2)
            {cx=parent.cx; cy=parent.cy+ny_new; ny_new++;}
            
            
          else if(j%4==3)
            {cx=parent.cx+nx_new; cy=parent.cy+ny_new;nx_new++;ny_new++;}



            
        fprintf(debug_file, "\n (%ld,%ld) (%ld+%ld-1=%ld,%ld) (%ld,%ld+%ld-1=%ld) (%ld+%ld-1=%ld,%ld+%ld-1=%ld)",cx,cy,cx,nx_new,cx+nx_new-1,cy,cx,cy,ny_new,cy+ny_new-1,cx,nx_new,cx+nx_new-1,cy,ny_new,cy+ny_new-1);
        int count=0;
  ...
  //  printf("here \n \n \n \n");
    ...
    encode_adaptive_wnc(cache_sym,    master_count,             num_sym+1,             0.3,           8192,             0,   fp);
    ...
    // printf("%d ",tree->level);
     ...
     for(i=(long)(tree->level)-1;i>=1;i--)
     {
      ...
      for( j = 0; j < n; j=j+4 ) {
        ...
        node3=tree->nodes[i][j+2];
        node4=tree->nodes[i][j+3];
        k+=(node1.value+node2.value+node3.value+node4.value)%4;
...
{   
   ...
   // for(int i=0;i<=num_sym;i++)
    ...
    alloc_bitbuffer(&buffer,(long)(pow(4.0,tree->max_level)/8)+1);
...
for( i = 1; i < (long)(tree->level); i++ ) {
    ...
    //  if(node.value==0)
    //  printf("\n %ld %ld %ld %ld", cx,cy,nx_new,ny_new);
...

{
  ...
  /*  sprintf(tmp_file_run,"%s.run",output_file);
   binary_file = bfopen(tmp_file_run,"w");
    ...
    createREEDTree(&tree,10);
    createREEDTree(&tree2,10);
...
//FILE* debug_file
    ...
    long num_sym,n;
    init_tree(&tree, image.orig_rgb[0],2,nx[0],ny[0],dfile,binary_file2, &num_sym, &n, cache);
   // init_tree(&tree, tmp_img,2,nx[0],ny[0],dfile,binary_file2, &num_sym, &n);
