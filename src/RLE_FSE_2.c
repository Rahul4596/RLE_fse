...
void encode_adaptive_wnc(
  ...
  for (i=0;i<n;i++) {
    ...
    for(j=0;j<s;j++)
      {
        fprintf(debug_file, "%ld ",counter[j] );
      }
      fprintf(debug_file," csum=%ld oldL=%ld oldH=%ld C=%ld",csum,L,H,C);
    }
...
FILE* debug_file, FILE* fp, long *num_symbols, long *source_length, long *norm_count, long *finalState) {

  long i,n,j,k,l,c,prev,flag=0;  /* loop variables */
  long cx,cy; 
  long num_sym=0,master_count=0;
...
//printf("%ld \n", pow(2,10));
  ...
  for( i = 1; i < (long)(tree->level); i++ ) {
    ...
    compress_fse(cache_sym, master_count, norm_count, fp, finalState);
...
int store_tree
    ( const REEDTree *tree,        /* pointer to tree */
      FILE *fp, long nx, long ny, long num_sym, long source_length, long *cache, long finalState)                  /* bitfile  to write to */
...
{   
    ...
    fwrite(&finalState, sizeof(long),1,fp);
    //fwrite(&tableLog, sizeof(long),1,fp);
...
int load_tree
    ( REEDTree *tree,        /* pointer to tree */
      FILE *fp, FILE *fp2, FILE *debug_file, long srcSize)              /* bitfile  to write to */
    /* write reed tree to bitfile,
       return 0 on success, 1 else. */
{
    ...
    fread(&initialState,sizeof(long),1,fp);
  ...
  //    fread(&(cache[i]),sizeof(long),1,fp);
    ...
    clock_t t,t_tot=0;
	t=clock();
	decompress_fse(fp2, cache_sym, initialState, srcSize  );
	t=clock()-t;
    ...
    n = pow( 2.0, i );
     ...
     // printf("\n %ld %ld %ld %ld %ld %ld %ld %ld", node.cx,node.cy,node.nx,node.ny, node.value,parent.cx,parent.cy,parent.nx,parent.ny);
      ...
      if(node.value==0)
      {
        ...
        {
          while(cache_sym[master_count]==254)
          {
            (tree->nodes[i][j]).run[k]+=254;
            master_count++;
          }
...
void compress_fse(long* in_buff, long srcSize, long *norm_count, FILE* output_file, long *finalState) {
...
long *count,total=0,total_norm=0,max=0,tableSize=1024,maxValue=256,tableLog=log2(tableSize);

//*logValue=tableLog;

//long norm_count[257];

alloc_vector_int(&count,257); // must change if to use for runlengths
...
for(i=0;i<maxValue;i++)
{
  ...
  //if(norm_count[i])
  //printf(" symbol=%ld dbits=%ld dstate=%ld count=%ld \n",i,symbolTT[i].dBits, symbolTT[i].dstate, norm_count[i]);
}
...
alloc_bitbuffer(&buffer,20000*8);
//printf("%ld \n",sizeof(*in_buff));
for(i=0;i<maxValue;i++)
{
  //if(count[i])
...
//out_buff[srcSize-1]=stateValue;
//*finalState=stateValue;
//printf("here\n");

//write_long_bitwise_2(srcSize,8*sizeof(long),output_file);
//write_long_bitwise_2(tableLog,8,output_file); //header info
//write_long_bitwise_2(stateValue,8*sizeof(unsigned),output_file);
...
for(i=srcSize-1;i>=0;i--)
  {
    ...
    symbolT=symbolTT[in_buff[srcSize-1-i]];
    nBitsOut = (unsigned)(out_buff[srcSize-1-i]+symbolT.dBits) >> 16;
    //if(i<100)
      //printf("\n %ld %ld", (unsigned)out_buff[srcSize-1-i],nBitsOut);
    write_long_bitwise_2(out_buff[srcSize-1-i],nBitsOut,&buffer);
...
void decompress_fse(FILE* input_file, long* out_buff, long initialState, long srcSize)
{
  ...
  //printf("decoded-> %ld %ld %ld \n", srcSize,tableLog, initialState);
  ...
  unsigned i,j;
  BITBUFFER buffer;
  alloc_bitbuffer(&buffer,8*srcSize);
  ...
  for(i=0;i<256;i++)
    {
      norm_count[i]=read_long_bitwise_2(&buffer,tableLog);
      for(j=0;j<norm_count[i];j++)  //iterating for the number of normalized occurences
    {
  ...
  for(k=0;k<srcSize;k++)
  {
    ...
    //if(k>= srcSize-500)
    //printf("%ld->%ld %ld %ld",k,out_buff[k], stateValue,decode_table[stateValue].nBits );
