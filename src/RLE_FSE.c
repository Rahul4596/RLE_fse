...
void compress_fse(long* in_buff, long srcSize, long *norm_count, BFILE* output_file) {
...
for(i=srcSize-1;i>=0;i--)
    ...
    if(i<100)
      printf("\n %ld %ld", (unsigned)out_buff[srcSize-1-i],nBitsOut);
		write_long_bitwise_2(out_buff[nx*ny-1-i],nBitsOut,output_file);
...
void decompress_fse(FILE* input_file, long* out_buff, long *norm_count)
{
...
FILE* debug_file, BFILE* fp, long *num_symbols, long *source_length, long *norm_count) {
...
/*  while(avg_thres>threshold)
 ...
 /*     for(k=0;k<master_count;k++)
  ...
  //  printf("here \n \n \n \n");
    ...
    //long finalState;
    //printf( "\n %ld ", master_count );
    compress_fse(cache_sym, master_count,  norm_count, fp);
...
int load_tree
    ( REEDTree *tree,        /* pointer to tree */
      FILE *fp, BFILE *fp2, FILE *debug_file, long *norm_count)              /* bitfile  to write to */
    /* write reed tree to bitfile,
       return 0 on success, 1 else. */
{
  ...
  //    fread(&(cache[i]),sizeof(long),1,fp);
    ...
    decompress_fse(fp2, cache_sym, norm_count );
    ...
    n = pow( 2.0, i );
     ...
     // printf("\n %ld %ld %ld %ld %ld %ld %ld %ld", node.cx,node.cy,node.nx,node.ny, node.value,parent.cx,parent.cy,parent.nx,parent.ny);
      ...
      if(node.value==0)
      {
        ...
        {
          while(cache_sym[master_count]==255)
          {
          	(tree->nodes[i][j]).run[k]+=255;
          	master_count++;
          }
...
//FILE* debug_file
   ...
   long num_sym,n;
   long norm_count[257];
    init_tree(&tree, image.orig_rgb[0],2,nx[0],ny[0],dfile,binary_file2, &num_sym, &n, norm_count);
    ...
    load_tree(&tree2,binary_file_in,binary_file_in2, dfile, norm_count);
